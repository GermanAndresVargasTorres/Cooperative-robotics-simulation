function [robot,secrobot] = I2seguidor(robot,secrobot)
...
%Sección 12 (Preparar movimiento)
elseif(secrobot==12)        
...
elseif(secrobot==13)
    ...
    for i=3:8                  
        if ((isalmost(cond1,mty(i),0.0025)) && ((i-3) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
...
elseif(secrobot == 15)
    ...
    %Si se detecta un marcador, activaremos la bandera
    for i=8:10                  
        if ((isalmost(cond1,mtx(i),0.0025)) && (isalmost(cond2,mty(i),0.0025)) && ((i-3) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
...
elseif(secrobot==16)
    ...
    %Si se detecta un marcador, activaremos la bandera
    for i=10:12                  
        if ((isalmost(cond1,mtx(i),0.0025)) && ((i-3) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
