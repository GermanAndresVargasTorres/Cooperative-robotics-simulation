function [robot,secrobot] = I4lider(robot,secrobot)
...
%Sección 33 (Preparar movimiento)
elseif(secrobot==33)        
    ...
    for i=13:18                  
        if ((isalmost(cond1,mty(i),0.0025)) && ((18-i) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
...
elseif(secrobot == 36)
    ...
    %Si se detecta un marcador, activaremos la bandera
    for i=11:13            
        if ((isalmost(cond1,mtx(i),0.0025)) && (isalmost(cond2,mty(i),0.0025)) && ((18-i) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
...
elseif(secrobot==37)
    ...
    %Si se detecta un marcador, activaremos la bandera
    for i=9:11                  
        if ((isalmost(cond1,mtx(i),0.0025)) && ((18-i) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
...
elseif(secrobot == 39)
    ...
    %Si se detecta un marcador, activaremos la bandera
    for i=7:9                  
        if ((isalmost(cond1,mtx(i),0.005)) && (isalmost(cond2,mty(i),0.0025)) && ((18-i) ~= robot.mcont))
            robot.ind = 1;
            break
        else
            robot.ind = 0;
...
elseif(secrobot==40)
    ...
    %Movimiento hacia adelante y reporte
    robot = h6(robot, 'Adelante', 3, 1);
    wmax=(4391/51)*(pi/30);
    robot = h1(robot, [1 1 1 1], [wmax wmax wmax wmax]);
...
elseif(secrobot==42)
    ...
    if ((isalmost(cond1,20,0.0025)) && (isalmost(cond2,4.444,0.0025)))
        robot.inf = 1;
    else
        robot.inf = 0;
    end
